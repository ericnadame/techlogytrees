...
export class BigIntDecimal implements DecimalClass {
  ...
  add(value: ValueType): BigIntDecimal {
    if (this.isInvalidate()) {
      return new BigIntDecimal(value || 0);
    }

    const offset = new BigIntDecimal(value || 0);
    if (offset.isInvalidate()) {
      return this;
    }

    const maxDecimalLength = Math.max(
      this.getDecimalStr()?.length ?? 0,
      offset.getDecimalStr()?.length ?? 0,
    );
...
export default function getMiniDecimal(
